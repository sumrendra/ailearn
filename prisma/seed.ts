...
async function main() {
  ...
  const sqlPath = await prisma.learningPath.upsert({
  ...
  // Flashcards — SQL Mastery Path
  await prisma.flashcard.createMany({
    skipDuplicates: true,
    data: [
      // L1: Relational model & SELECT
      {
        lessonId: sqlL1.id,
